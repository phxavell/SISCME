import { RelatorioProcessosAPI } from "@/infra/integrations/gerente/relatorio-processos"
...
export const useRelatoriosProcessos = (clientes?: any) => {
	...
	const handleBuscarRelatorios = useCallback( () => {
		setLoading(true)
		RelatorioProcessosAPI.listarRelatorios(user, paramsMemo).then((resposta) => {
			setLoading(false)
			setDados(resposta)
		}).catch((error) => {
			setLoading(false)
