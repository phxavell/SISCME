...
import {makeChart, mapLabelsCliente} from "./helper.ts"
...
export const useGraficoGeral = (props: any) => {
	...
	useEffect(() => {
		...
		dados?.forEach((cliente: any) => {
			maiorNumero = Math.max(maiorNumero, cliente.quantidade)
		})
		...
		const options: any = {
			plugins: {
				...
				title: {
					display: true,
					text: `Quantidade de etiquetas geradas no preparo por cliente`,
					font: {
						size: 20
					}
				}
