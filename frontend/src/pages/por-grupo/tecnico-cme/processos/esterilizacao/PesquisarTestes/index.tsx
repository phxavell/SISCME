...
import { usePesquisarTestes } from "./usePesquisarTestes"
...
const CustomizedMarker = (item: TimelineEvent) => {
    ...
    `
	return (
		<div className={style} style={{ backgroundColor: item.color }}>
			<i style={{ height: `15px` }} className={`${item.icon} iconI`}></i>
		</div>
	)
}
const CustomizedContent = (item: TimelineEvent) => {
	return <div className="text-xs mt-0 gray-900 flex ml-0">{item.status}</div>
}

export const PesquisarTestes = () => {
	...
	const { getEquipamento, equipamento } = useModalEquipamentoView()
	...
	const customBodyTimeline = (esterilizacao: any) => {
		...
		const events = [
			{
				date: moment(esterilizacao.data_inicio, formateDateFrom).format(
					formateDateFinal,
				),
				status: `Iniciado`,
		...
		if (esterilizacao.situacao_atual === `ESTERILIZACAO_INICIO`) {
			events.push({
				...
				icon: `pi pi-clock`,
				color: `blue`,
				date: `Há ${moment(esterilizacao.data_inicio, formateDateFrom)
					.locale(`pt-br`)
					.fromNow(true)}`,
			})
		}
		...
		if (esterilizacao.situacao_atual === `ESTERILIZACAO_FIM`) {
			events.push({
				date: moment(esterilizacao.data_fim, formateDateFrom).format(
					formateDateFinal,
				),
				status: `Finalizado`,
				icon: `pi pi-check-circle`,
	...
	return (
		<div className={ContainerColumnPx}>
			<TitleWithBackArrow
				title={`Pesquisa de Testes Bowie & Dick`}
