...
import { EMensagensNulas, EtiquetaFormSchemaType } from "../schemas"
...
import { ModalEtiquetaPdf } from "../modais/ModalEtiquetaPdf"
import { ModalComplemento } from "../modais/ModalComplemento"
import { ModalEdicaoProps } from "./types"
import { styleRadios } from "@pages/por-grupo/administrativo/cruds/motorista/novo-motorista/ModalMotorista/styles.ts"
import { useModalEdicao } from "./useModalEdicao"
import { useAuth } from "@/provider/Auth"
import {SetorAPI, SetoresProps} from "@infra/integrations/setor.ts"

export const ModalEtiquetaEdicao = ({
	visibleEdicao,
	onClose,
	etiquetaEdicao,
	setEtiquetaEdicao,
	...
	} = useModalEdicao(visibleEdicao, etiquetaEdicao)

	const { user, toastError } = useAuth()


	const [visibleModalComplemento, setVisibleModalComplemento] =
        useState(false)
	...
	}, [handleBuscarSetores,visibleEdicao])

	const handleVisualizarEtiqueta = useCallback(
		(data: any) => {
			setEtiqueta(data)
			setVisibleModalPdf(true)
		},
		[setEtiqueta, setVisibleModalPdf],
	)

	const handleAtualizarComplemento = useCallback(
		async (data: any) => {
			await listarComplementos()
			setValue(`idcomplemento`, data?.id)
		},
		[listarComplementos, setValue],
	)

	const handleAtualizarSetor = useCallback(
		async (data: any) => {
			await handleBuscarSetores()
			setValue(`setor`, data.id)
			setVisibleModalSetor(false)
		},
		[handleBuscarSetores, setValue, setVisibleModalSetor],
	)
	const handleEtiqueta = useCallback(
		(dataPayload: EtiquetaFormSchemaType) => {
			const payload = {
				...dataPayload,
				idcli: cliente,
			}

			if (etiquetaEdicao?.id) {
				EtiquetaAPI.atualizarEtiqueta(user, payload, etiquetaEdicao.id)
					.then((data) => {
						handleVisualizarEtiqueta(data)
						onClose()
						setEtiquetaEdicao(undefined)
						reset()
					})
					.catch((error: any) => {
						if (error.data) {
							...
							toastError(
								`Não foi possível atualizar etiqueta!!`,
								false,
							)
						}
					})
	...
	}, [isDirty, onClose, setMiniModalVisible, setEtiquetaEdicao])

	useEffect(() => {
		const handleKeyDown = (e: any) => {
			if (e.key === `Escape` && visibleEdicao) {
	...
	}, [visibleEdicao, confirmarFecharModal])

	const headerModalTemplate = useCallback(() => {
		return (
			<div className="flex  w-full justify-content-end h-3rem">
				<h3 className="text-md md:text-2xl my-auto">
                    Atualizar Etiqueta
				</h3>
				<Button
					loading={isLoading}
	...
	}, [isLoading, handleSubmit, handleEtiqueta])
