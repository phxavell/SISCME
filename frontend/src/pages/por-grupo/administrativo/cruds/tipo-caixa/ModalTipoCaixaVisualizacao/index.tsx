...
import { TipoCaixaProps } from "../schemas"
import RenderObject from '@/components/RenderObject'

export interface ModalPropsTipoCaixa {
    visible: boolean
    onClose: (prop: boolean) => void
    tipoCaixaData: TipoCaixaProps
}
export function ModalTipoCaixaVisualizacao({ visible, onClose, tipoCaixaData }: ModalPropsTipoCaixa) {

	const idSetor = `#${tipoCaixaData.id}`
	const dataAtualizacao = moment(tipoCaixaData.atualizado_em)
		.format(`DD/MM/YYYY HH:mm`)
	const dataCriacao = moment(tipoCaixaData.criado_em)
		.format(`DD/MM/YYYY HH:mm`)

	const criadoPor = tipoCaixaData?.criado_por

	const atualizadoPor = tipoCaixaData?.atualizado_por
	...
	return (
		<Dialog
			header={idSetor}
			visible={visible}
			data-testid="modal-visualizar-tipos-caixa"
			style={{ width: `50vw` }}
			draggable={false}
			resizable={false}
			onHide={() => {
			...
			<div className="w-full">
				<div className="w-full flex flex-row ">
					<div className="w-full flex flex-column gap-3">
						<h3 className="flex flex-row p-0 m-0">Descrição: <div className="ml-2 font-normal">{tipoCaixaData?.descricao}</div></h3>
						{CriadoPor}
						{AtualizadoPor}
					</div>
				</div>
